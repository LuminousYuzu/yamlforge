...
def main():
    """Main function to demonstrate the YAML reader."""
    ...
    print("🔍 Searching for YAML files in the repository...")
    ...
    if not yaml_files:
        ...
        print("You can also manually specify a YAML file path to read.")

        # Example: Try to read a common YAML file
        common_yaml_files = [
            "config.yml",
            "config.yaml",
            "data.yml",
        ...
        for file_path in common_yaml_files:
            print(f"\n📄 Trying to read: {file_path}")
            data = reader.read_and_parse_yaml(file_path)
            if data:
                print(f"✅ Successfully parsed {file_path}:")
                print("=" * 50)
                reader.print_parsed_info(data)
                print("=" * 50)
                break
        else:
            print(
                "❌ No YAML files could be read. Please check the repository structure."
            )
    else:
